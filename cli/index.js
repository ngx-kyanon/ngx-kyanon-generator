...
const addonKey = 'ngx-kyanon-addon';
const disabledAddons = 'disabledAddons';
const blacklistedNpmAddons = ['ngx-kyanon-generator', 'ngx-kyanon-generator-addon'];
...
class NgxCli {
  constructor(args) {
    ...
    this._config = new Conf({
      projectName: 'ngx-kyanon-generator',
      defaults: {
        disabledAddons: {}
      }
    });

    env.register(require.resolve('..'), 'ngx-kyanon');
  }
  ...
  async generate(update, args, options) {
    ...
    if (!update) {
      ...
      addon = Boolean(get(rc, 'ngx-kyanon-generator.props.isAddon'));
    } else {
    ...
    if (addon) {
      ...
      env.lookup(() =>
        env.run(['ngx-kyanon:addon'].concat(args), {
          update,
  ...
  _packageManager() {
    ...
    try {
      ...
      pm = rc['ngx-kyanon-generator'].props.packageManager;
    } catch (_) {
      // Do nothing
    }
